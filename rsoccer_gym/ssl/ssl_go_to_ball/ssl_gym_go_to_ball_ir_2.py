...
class SSLGoToBallIREnv2(SSLBaseEnv):
    """The SSL robot needs to reach the ball 
        ...
        Observation:
            ...
            11       min angle between robot and ball
    ...
    def _frame_to_observations(self):
        ...
        for i in range(self.n_robots_blue):
            ...
            # add robot angle to ball
            observation.append(self.__calclulate_angle([self.frame.robots_blue[i].x, self.frame.robots_blue[i].y],
                                                        np.deg2rad(self.frame.robots_blue[i].theta),
    ...
    def _calculate_reward_and_done(self):
        ...
        if not done:
            if robot.infrared:
                ...
                self.reward_shaping_total['contact'] += 1
            elif self.last_frame is not None:
                diff_ball_dist_r = self.__ball_dist_r() / self.ball_dist_scale
                diff_robot_angle_ball = self.__robot_angle_ball()

                R_distance = (diff_ball_dist_r + diff_robot_angle_ball) / 2

                self.reward_shaping_total['distance'] += R_distance
                reward = reward\
                        + R_distance
    ...
    def __ball_dist_r(self):
        ...
        return last_ball_dist - ball_dist
    
    def __robot_angle_ball(self):
        assert(self.last_frame is not None)
        # Calculate previous robot to ball angle
        last_ball = self.last_frame.ball
        last_robot = self.last_frame.robots_blue[0]
        last_ball_pos = np.array([last_ball.x, last_ball.y])
        last_robot_pos = np.array([last_robot.x, last_robot.y])
        last_robot_theta = np.deg2rad(last_robot.theta)
        last_angle = self.__calclulate_angle(last_robot_pos, last_robot_theta, last_ball_pos)

        # Calculate new robot to ball angle
        ball = self.frame.ball
        robot = self.frame.robots_blue[0]
        ball_pos = np.array([ball.x, ball.y])
        robot_pos = np.array([robot.x, robot.y])
        robot_theta = np.deg2rad(robot.theta)
        angle = self.__calclulate_angle(robot_pos, robot_theta, ball_pos)

        return last_angle - angle
    
    def __calclulate_angle(self, robot_pos, theta, ball_pos):
        xa = 2 * math.cos(theta) + robot_pos[0]
        ya = 2 * math.sin(theta) + robot_pos[1]

        a = math.sqrt((ball_pos[1] - robot_pos[1])**2 + (ball_pos[0] - robot_pos[0])**2)
        b = math.sqrt((ball_pos[1] - ya)**2 + (ball_pos[0] - xa)**2)
        c = math.sqrt((robot_pos[1] - ya)**2 + (robot_pos[0] - xa)**2)

        eq = (a**2 + c**2 - b**2) / (2 * a * c)
        if eq > 1:
            eq = 1
        if eq < -1:
            eq = -1
        ...
        return self.__norm_angle(angle)
    
    def __norm_angle(self, angle):
